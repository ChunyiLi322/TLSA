...
def train(episode):
    ...
    if episode % log_interval == 0:
       #print("-------------------1----------------", np.array(predict.detach().numpy()))
       #print("-------------------1----------------", np.array(target.detach().numpy()))
       #print('Weighted precision', precision_score(y_true = np.array(predict.detach().numpy()),y_pred = np.array(target[0:predict.shape[0]].detach().numpy()), average='micro'))
       #print('Weighted recall', recall_score(predict.detach().numpy(), target[0:predict.shape[0]].detach().numpy(), average='weighted'))
       #print('Weighted f1-score', f1_score(predict.detach().numpy(), target[0:predict.shape[0]].detach().numpy(), average='weighted'))    
       log_record = open("log_500_acc.txt", mode = "a+", encoding = "utf-8")
       log_record2 = open("log_500_loss.txt", mode = "a+", encoding = "utf-8")
       print('Train Episode: {} Loss: {} Acc: {}'.format(episode, loss.item(), acc))
       acc = '{}'.format(acc)
       loss = '{}'.format(loss.item())
       episode = '{}'.format(episode)
       print(episode+", "+ acc, file = log_record)
       print(episode+", "+ loss, file = log_record2)
       log_record.close()
       log_record2.close()
